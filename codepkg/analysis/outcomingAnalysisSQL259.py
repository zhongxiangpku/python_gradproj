...
def outputRPlotData(city1,city2,file):
    generateOutComingVector(city1,xMap,city1, city2)
    generateOutComingVector(city2, yMap, city1, city2)
...
def calculateAllIncommingSimilarity():
    insertOutSimilaritySQL = "insert into outsimilarity_259(fromcity,tocity,similarity,createtime,updatetime) values(%s, %s, %s, %s,%s)"
    db = MySQLdb.connect(mod_config.dbhost, mod_config.dbuser, mod_config.dbpassword, mod_config.dbname,
                         charset='utf8')
    cursor = db.cursor()
    count = 0
    ...
    queryAllUidsSQL = "select fromcity,tocity from outsimilarity_259 "
...
def getMappingSimilarityDataFromSQL(city,outfile):
    ...
    mysql = 'select tocity,similarity from outsimilarity_259 where fromcity ="'+city+'"'
    cursor.execute(mysql)
    results = cursor.fetchall()
    similarityMap = {}
    for row in  results:
...
queryCity = r'西安'
outputSimilarityFilePath = pwd+'\\Datas\\similarity_datas\\output_outcomming_Similarity_xian259'+'.txt'
